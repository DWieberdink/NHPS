...
document.addEventListener("DOMContentLoaded", () => {
  ...
  svg.call(d3.zoom().scaleExtent([0.5, 2]).on("zoom", e => g.attr("transform", e.transform)))
     .call(d3.zoom().transform, d3.zoomIdentity.scale(0.8).translate(100, 50));
...
//Draw Lines (Links)
  ...
  console.log("🔍 link-labels found?", !d3.select(".link-labels").empty());
  ...
  window.addEventListener("load", () => {
    ...
    const content = svg.querySelectorAll(".node, .link, .link-label");

    if (!content.length) return;

    const tempGroup = document.createElementNS("http://www.w3.org/2000/svg", "g");
    content.forEach(el => tempGroup.appendChild(el.cloneNode(true)));
    svg.appendChild(tempGroup);

    const box = tempGroup.getBBox();
    ...
    complete: function (res) {
      ...
      if (select) {
        schoolData.forEach(row => {
          ...
          opt.value = row["Building Name"];
          opt.textContent = row["Building Name"];
          select.appendChild(opt);
        });
        const first = schoolData[0]?.["Building Name"];
        if (first) {
          select.value = first;
          const { path, decisions } = evaluatePath(
            schoolData.find(r => r["Building Name"] === first),
            window.thresholds
          );
          highlightFlow(path, decisions);
        }
